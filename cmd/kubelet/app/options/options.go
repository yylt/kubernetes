...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.StringSliceVar(&c.ImageGCWhitelist, "image-gc-whitelist", c.ImageGCWhitelist, "Comma-separated list of images which will be ignored by image garbage collection")
	...
	fs.StringVar(&c.KubePodsCpusetCpus, "kubepods-cpuset.cpus", c.KubePodsCpusetCpus, "CPUs in which to allow execution (0-3, 0,1)")
	fs.StringVar(&c.KubePodsCpusetMems, "kubepods-cpuset.mems", c.KubePodsCpusetMems, "MEMs in which to allow execution (0-3, 0,1)")
